...
const headerControl = headerContainer.childNodes[1]
let prevScrollTop = 0;

const headerMoving = function(direction){
  if(direction === 'up'){
    headerControl.classList.remove('hide_header');
    trgAside.classList.remove('hide_trg');
    trgAside.style.transition = '0.5s'
  }else if(direction === 'down'){
    headerControl.classList.add('hide_header');
    trgAside.classList.add('hide_trg');
    headerControl.style.borderBottom = 'none';
  }else if(direction === 'default'){
      headerControl.style.borderBottom = '3px solid #474747';
  }
};
...
trgAside.addEventListener('click', () => {
    if(body.classList.contains('asideOpen') == false){
      console.log('check');
      body.classList.add('asideOpen');
    }else if(body.classList.contains('asideOpen') && headerControl.classList.contains('hide_header') == false){
      body.classList.remove('asideOpen');
      for(let i = 0; i < closeMenu.length; i++){
      closeMenu[i].classList.remove('open');
      }
    }
    if(trgAside.classList.contains('hide_trg') == false && headerControl.classList.contains('hide_header')){
      body.classList.remove('asideOpen');
      trgAside.classList.add('hide_trg');
      for(let i = 0; i < closeMenu.length; i++){
        closeMenu[i].classList.remove('open');
...
confirmBtn.addEventListener('click', () => {
  body.classList.add('asideOpen');
  trgAside.classList.remove('hide_trg');
  trgAside.style.transition = 'none';
  closeMenu[8].classList.add('open');
})
...
(function($){
	...
	newriver.slider = (function(_){
		return {
			...
			contentsItemSlider : function(){
				var $contentsSlider = $('.swiper-wrapper');
				alert("ddd");
				$contentsSlider.slick({
					fade : false,
					arrows : true,
			...
			travelogSlider : function(sliderStartNum){
				var $travelogSlider = $('.travelog_slider');

				$travelogSlider.slick({
					fade : false,
					arrows : true,
					dots : true,
					infinite : true,
					initialSlide : sliderStartNum - 1 ? sliderStartNum - 1 : 0
				});
			}
		}
	})(newriver);
	...
	newriver.loginAction = function(){
		...
		$('input, textarea, select').on('focus', function(){
			$('.btn120').css('bottom', '88px');
			$('.bottom').css('bottom', '88px');
			$('#gnb').css('padding-bottom', '300px');
		});
		*/
